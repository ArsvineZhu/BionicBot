...
Bionic Bot是一款基于ncatbot框架开发的智能QQ机器人，集成了火山方舟AI模型，支持群聊和私聊场景，能够根据上下文生成连贯、智能的回复。
...
|-----------|------|------|
| Python | 3.10+ | 开发语言 |
| ncatbot | 4.3.3.post2+ | QQ机器人框架 |
| volcengine-python-sdk[ark] | 4.0.0+ | AI模型接口 |
| pyyaml | 6.0+ | YAML配置管理 |
...
1. **Python环境**：确保安装了Python 3.10或更高版本
2. **虚拟环境**：建议使用虚拟环境隔离项目依赖
3. **QQ账号**：需要一个QQ账号作为机器人账号
4. **ncatbot框架**：通过pip自动安装
...
配置文件采用YAML格式，位于 `bot/data/config/` 目录，支持动态调整，无需重启机器人即可生效。配置系统实现了安全可靠的加载机制，当配置文件不存在、解析错误或权限问题时，会使用合理的默认值继续运行。
...
```
用户: yuki，你好
机器人[2025-12-12 22:45]: 你好呀！有什么可以帮助你的吗？
```

#### @触发
```
用户: @AI助手 今天天气怎么样？
机器人[2025-12-12 22:46]: @用户 今天天气晴朗，温度适宜，适合外出活动！
```

#### 昵称-称呼映射
```
用户: Texas: 你好
机器人[2025-12-12 22:47]: A酱，你好呀！有什么可以帮助你的吗？
```
...
```
用户: 你能做什么？
机器人[2025-12-12 22:48]: 我是AI助手，一个智能QQ机器人，可以陪你聊天、回答问题、提供各种帮助。
用户: 那你知道今天是星期几吗？
机器人[2025-12-12 22:49]: 今天是2025年12月12日，星期五。
```
...
1. **添加新的配置项**
   - 在 `bot/config/settings.py` 的 `DEFAULT_CONFIG` 中添加默认值
   - 在 `BotSettings` 类中添加对应的配置项

2. **添加新的回复模式**
   - 在 `bot/core/tracker.py` 中添加新的回复模式逻辑
   - 更新 `should_respond` 方法中的判断逻辑

3. **添加新的功能模块**
   - 在 `bot/core/` 目录下创建新的模块文件
   - 在对应的 `__init__.py` 中导出模块
   - 在需要的地方导入使用

4. **添加新的消息处理逻辑**
   - 在 `bot/handlers/` 目录下扩展现有的处理器
   - 或创建新的处理器类

### 测试指南

1. **配置系统测试**
   - 使用 `bot/core/test_config_system.py` 测试配置加载和管理
   - 运行 `python -m bot.core.test_config_system`

2. **功能单元测试**
   - 使用 `bot/utils/unit_test/` 目录下的测试脚本进行单元测试
   - 编写新的测试用例覆盖新增功能

### 调试建议

1. **日志查看**：查看控制台输出的日志信息，了解机器人运行状态
2. **配置调试**：修改配置文件中的配置项，重启机器人查看效果
3. **单元测试**：使用 `bot/utils/unit_test/` 目录下的测试脚本进行单元测试
4. **代码调试**：使用IDE的调试功能，设置断点查看变量值和执行流程
